...
make_debug_archive () {
  ...
  if [ "${CI_COMMIT_BRANCH:-}" = "master" ]; then
    ...
    curl --fail https://storage.kde.org/ci-artifacts/kde-linux/kde-linux-packages/j/4683887/testing/artifacts/debug.tar.zst \
      | zstd --decompress | tar --extract --directory=/var/tmp/debugroot
  fi
...
if [ "${CI_COMMIT_BRANCH:-}" = "master" ]; then
  ...
  wget --output-document=install.tar.zst https://storage.kde.org/ci-artifacts/kde-linux/kde-linux-packages/j/4683887/testing/artifacts/install.tar.zst
fi
